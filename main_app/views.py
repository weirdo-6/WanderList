...
class TravelCreate(LoginRequiredMixin, CreateView):
    model = Travel
    fields = ['name', 'country', 'city', 'description', 'image', 'completed']
...
def search_trips(request):
    ...
    planned_travels = Travel.objects.filter(
        user=request.user,
        completed=False
    )
    completed_travels = Travel.objects.filter(
        user=request.user,
        completed=True
    )

    if query:
        planned_travels = planned_travels.filter(
            Q(name__icontains=query) |
            Q(country__icontains=query) |
            Q(city__icontains=query)
        )
        completed_travels = completed_travels.filter(
            Q(name__icontains=query) |
            Q(country__icontains=query) |
            Q(city__icontains=query)
        )
    ...
    context = {
        'planned_travels': planned_travels,
...
def planned_trips(request):
    travels = Travel.objects.filter(user=request.user, completed=False)
    return render(request, 'travels/planned_trips.html', {'travels': travels})

@login_required
def completed_trips(request):
    travels = Travel.objects.filter(user=request.user, completed=True)
    return render(request, 'travels/completed_trips.html', {'travels': travels})
...
class PlannedTripCreate(LoginRequiredMixin, CreateView):
    model = Travel
...
class CompletedTripCreate(LoginRequiredMixin, CreateView):
    model = Travel
    fields = ['name', 'country', 'city', 'description', 'image']
